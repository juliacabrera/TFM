...
dt = 0.005 ; % Pas de temps (en hores). (0.005)
iter = 100000 ; % Iteracions de temps que volem calcular
D = 0.05e-3 ; % Coeficient de difusió (en mm2/h) (ESTAVA A 0.1e-3)(0.05e-3)
a = 0.0; % mateixes unitats que la concentració (g/mm3) (0.13)
k = 0.2 ; % S'ha de comprovar amb el valor de l'article (en mm3/h g)?
initial_concentration = 0.7 ; %(g/mm3)
tau = 120.5; % Proportionality factor for which we multiply k and D to adjust dyamics (120.5)
...
c0 = 3.0*ones(d,d,d) ; %23 funciona prou bé pero massa gran per fibriblast
eps = 2.0;
...
gradient_c = ((gx.*gx) + (gy.*gy) + (gz.*gz)).^1.0;
D_fib = 5000*D.*gradient_c;
...
%  v = VideoWriter('Ev_lesio_3d_inestable_costat_50000.mp4');
%  open(v);
...
for t=1:iter
    ...
    [c_post,dif_lesio] = lesion_inestable(d,D_les,a,k,dt,dx,tau,c,fib,g);
    c = c_post;
    ...
    [fib_post,dif_fib] = fib_inestable(d,D_fib,dtf,dx,fib,g);
    fib = fib_post;
...
function [m_post,DIF] = lesion_inestable(d,D,a,k,dt,dx,tau,c,m_fib,g)
    m_post = zeros(d,d,d);
    ix = 1:d-1;
    DIF = zeros(d,d,d);
    D = D/tau;
    ...
    m_post = c + dt*((k/tau)*c.*(1.0-m_fib-c) + (DIF.*D)/(dx^2.0));
end



function [dif_post,DIF] = fib_inestable(d,D,dt,dx,fib,g)
